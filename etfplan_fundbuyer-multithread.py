...
#                  11/24/2017 add multithread by thomas
...
#主程序的多线程实现 by thomas 11/24/17
def fund_analyst_thread(fundid):
    failed_funds= []
    retry=0
    suggestion1 = "" 
    suggestion2 = ""
            ...
            if (current_price != "--"):
                ...
                if (len(eda_prices)>0):
                    ...
                    if(count>0):
                        ...
                        print(suggestion2)
    return (suggestion1, suggestion2, failed_funds)    
...
#将结果作为入参生产建议 by thomas 11/24/17
def make_report(suggestion1,suggestion2,failed_funds):
    if len(failed_funds) != 0:
        print "因数据异常,以下基金未做分析，如有需要，请重新运行本程序："
        for fundid in failed_funds:
            fundid = "".join(fund)
...
class fundthread(threading.Thread):  
    ...
    def run(self):  
        self.result = fund_analyst_thread(self.fundid)  
...
#多线程循环调用begin  by thomas 11/24/17
ss1 = "" 
ss2 = ""
ff = []
threadpool=[]
...
for th in threadpool :
    s1, s2, f = th.get_result()
    ss1 += s1
    ss2 += s2
    if len(f) != 0:
        ff.append(f)
make_report(ss1,ss2,ff)
#多线程循环调用end

time_end = time.time()
# print "运行时间： %.2fsec" %(time_end - time_start)
print(u"运行时间： %.2fsec" %(time_end - time_start))
